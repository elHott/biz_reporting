...
"""
    cur.execute('SELECT count FROM PCNs WHERE org = ? ', (org_domain,))
    row = cur.fetchone()
    if row is None:
        cur.execute('''INSERT INTO PCNs (org, count)
                VALUES (?, 1)''', (org_domain,))
    else:
        cur.execute('UPDATE PCNs SET count = count + 1 WHERE org = ?',
                    (org_domain,))
"""
...
"""
print('Analyzing PCNs')
# first input file is a tab delimited dump of PCNs from MSP
...
for line in fh:
    if line.startswith('Y'):
        ...
        on_leave = pieces[31]
        count = count + 1
        print('PCN:', position_number)
        print('Email:', email)
        print('Reports to Level 2:', reports_to_level_2)
        print('Reports to Level 3:', reports_to_level_3)
        print('Function Exec Summary:', func_exec_summary)
        print('Cost Center:', cost_center)
        print('COGS/Opex:', cogs_opex)
        print('Open Position:', open_position_ind)
        print('Future Start:', future_start_date)
